...
async def analyze_excel(
    ...
    try:
        ...
        sentiment_counts = df["Sentiment"].value_counts().reindex(sentiment_order, fill_value=0).to_dict()

        top_feedback = {
            "Positive": df.loc[df["Sentiment"] == "Positive", "text"].head(5).tolist(),
            "Negative": df.loc[df["Sentiment"] == "Negative", "text"].head(5).tolist(),
        }
        ...
        bars = plt.barh(sentiment_order, list(sentiment_counts.values()), color=["#4CAF50", "#FFC107", "#F44336"])
        plt.xlabel("Count")
        plt.title("Sentiment Distribution")
        for bar, count in zip(bars, list(sentiment_counts.values())):
        ...
        full_pdf_url = f"http://127.0.0.1:8000/reports/{rel_pdf_path}"
        ...
        def safe_multicell(pdf_obj, text, max_chars_per_line=90):
            text = ''.join(c if c.isprintable() else '?' for c in text)
            for line in text.split("\n"):
                while len(line) > 0:
                    chunk = line[:max_chars_per_line]
                    pdf_obj.multi_cell(0, 6, chunk, ln=True)
                    line = line[max_chars_per_line:]

        safe_multicell(pdf, f"Event Name: {eventName}")
        safe_multicell(pdf, f"Organizing Club: {club}")
        safe_multicell(pdf, f"Description: {description}")
        safe_multicell(pdf, f"Date: {date}")
        safe_multicell(pdf, f"Participation Strength: {strength}")
        safe_multicell(pdf, summary_text)

        # --- Chart page ---
        pdf.add_page()
        pdf.set_font("Arial", "B", 12)
        pdf.cell(0, 10, "Sentiment Distribution", ln=True, align="C")
        pdf.ln(10)
        img_width = 120
        x_pos = (pdf.w - img_width) / 2
        pdf.image(temp_chart_path, x=x_pos, y=40, w=img_width)
